    ...
    with col1:
        ...
        if submit_button:
            ...
            for i in range(0, len(encounter.index)):
                ...
                st.markdown(f' **HP** {encounter["HP"].iloc[i]}; **AC** {encounter["AC"].iloc[i]}; **Fortitude** +{encounter["Fort"].iloc[i]}; **Reflex** +{encounter["Ref"].iloc[i]}; **Will** +{encounter["Will"].iloc[i]}')
                ...
                with spells_expander:
                    for j in range(1, 4):
                        if not pd.isna(encounter[f'Spells {j}'].iloc[i]):
                            st.markdown(f'Spells {j} = {encounter[f"Spells {j}"].iloc[i]}')
with col2:
    try:
        st.markdown('---')
        st.markdown(f':blue-background[{encounter["name"].iloc[0]}]')
    except:
        st.markdown('No encounter created yet.')